...
import { getToken, syncState } from '@/lib/api-client';
...
export default function Pricing() {
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '48px 24px' }}>
      ...
      {/* Back */}
      <button onClick={() => router.back()} style={{ position: 'absolute', top: 24, left: 24, background: 'none', border: 'none', color: 'var(--text3)', cursor: 'pointer', fontSize: 13, fontFamily: 'var(--font-mono)', display: 'flex', alignItems: 'center', gap: 6 }}>
        ← back
      </button>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 16, width: '100%', maxWidth: 640, marginBottom: 32 }}>
        ...
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: 24 }}>
          ...
          <button onClick={() => router.push('/dashboard')} style={{ width: '100%', padding: '12px', background: 'none', border: '1px solid var(--border)', borderRadius: 'var(--radius-sm)', color: 'var(--text2)', fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 14, cursor: 'pointer' }}>
            Continue free
          </button>
        </div>
        ...
        <div style={{ background: 'var(--surface)', border: '1px solid rgba(168,255,62,0.35)', borderRadius: 'var(--radius)', padding: 24, position: 'relative', overflow: 'hidden' }}>
          ...
          <button onClick={handleUpgrade} disabled={loading} style={{
            ...
            {loading ? 'Redirecting…' : `Start Pro${interval === 'annual' ? ' — Best Value' : ''} →`}
          </button>
        </div>
      </div>
